<script context="module">
	//////////////////////////////////////////////////////////////////////////
	// Credit to https://github.com/ONSvisual/svelte-scrolly 							  //
...
<script>
  ...
  let titleContainer; // IE patch. Container binding to update inline style
	let foreground;
	let background;
	let left;
	let sections;
  ...
  function setFixed() {
    if (bgContainer) {
      ...
      bgContainer.style.cssText = style;
			...
			if (titleContainer) {
				titleContainer.style.cssText = `position: ${fixed ? 'fixed' : 'absolute'}; top: 0; width: ${width}px; z-index: 3;`;
			}
    }
  }
...
<style>
  ...
  @media (min-width: 992px) {
    :global(.splitscreen svelte-scroller-background) {
    ...
    :global(.splitscreen [slot="foreground"] .col-medium) {
      width: 100%;
      margin: 0;	
      padding: 0 30px;
    }
